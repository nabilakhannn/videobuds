...
def generate_ugc_video(prompt, image_url=None, image_path=None, model=None,
    ...
    Args:
        ...
        image_url: Source image URL (start frame).  If this looks like a
                   local file path it is automatically swapped to *image_path*.
        image_path: Local file path to source image (alternative to image_url).
                    Supported natively by the Google Veo provider.
    ...
    # Auto-detect: if image_url is actually a local file path, swap it
    if image_url and not image_url.startswith(("http://", "https://", "data:")):
        # Resolve /api/outputs/<filename> web paths to local files
        if image_url.startswith("/api/outputs/"):
            from .config import PROJECT_ROOT
            local_name = image_url.replace("/api/outputs/", "")
            resolved = os.path.join(PROJECT_ROOT, "references", "outputs", local_name)
            if os.path.isfile(resolved):
                image_path = resolved
                image_url = None
                print_status(f"Resolved web path → {resolved}")
    ...
    if image_url:
        print_status(f"Source image URL: {image_url[:60]}...")
    elif image_path:
        print_status(f"Source image file: {os.path.basename(image_path)}")

    task_id = provider_module.submit_video(
        prompt, image_url=image_url, image_path=image_path, model=model,
        duration=duration, mode=mode, aspect_ratio=aspect_ratio
    )
    print_status(f"Task created: {task_id}", "OK")

    # Choose appropriate timeout — scale with duration for longer videos
    dur_int = int(duration) if str(duration).isdigit() else 5
    base_wait = 900 if model in ("sora-2", "sora-2-pro", "veo-3.1") else 600
    # Add extra time for longer videos: +60s per 5s above 10s
    extra = max(0, (dur_int - 10)) * 12
    max_wait = base_wait + extra
    poll_fn = getattr(provider_module, "poll_video", None)
    if poll_fn is None:
        raise RuntimeError(
            f"Provider '{provider_name}' does not support poll_video()"
        )
    return poll_fn(task_id, max_wait=max_wait, poll_interval=10)
